...
import { startAddClient, startDeleteClient, startGetAllClients } from '@stores/clients/actions/clients.actions';
...
export class ClientsVpnComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.actions$.pipe(
      filter((action: any) => action.type === '[CLIENTS] COMPLETE_ADD_CLIENT'))
      .subscribe((e) => {
    ...
    this.actions$.pipe(
      filter((action: any) => action.type === '[CLIENTS] ERROR_ADD_CLIENT'))
      .subscribe((e) => {
    ...
    this.actions$.pipe(
      filter((action: any) => action.type === '[CLIENTS] COMPLETE_DELETE_CLIENT'))
      .subscribe((e) => {
    ...
    this.actions$.pipe(
      filter((action: any) => action.type === '[CLIENTS] ERROR_DELETE_CLIENT'))
      .subscribe((e) => {
  ...
  addClient() {
    ...
    dialogRef.afterClosed().subscribe((e) => {
      ...
      const act = startAddClient({ vpn: this.name, clientId: e.clientId, purposes: purposes });
      this.store.dispatch(act);
    });
  }

  removeClient(client: ClientResult) {
    const act = startDeleteClient({ id: client.id});
    this.store.dispatch(act);
  }

  private refresh() {
    this.name = this.activatedRoute.parent?.snapshot.params['name'];
    const act = startGetAllClients({ vpn: this.name });
    this.store.dispatch(act);
  }
}
