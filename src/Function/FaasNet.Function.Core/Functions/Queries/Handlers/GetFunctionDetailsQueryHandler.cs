...
namespace FaasNet.Function.Core.Functions.Queries.Handlers
{
    public class GetFunctionDetailsQueryHandler : IRequestHandler<GetFunctionDetailsQuery, JObject>
    {
        ...
        public GetFunctionDetailsQueryHandler(IFunctionRepository functionRepository, IHttpClientFactory httpClientFactory, IOptions<FunctionOptions> configuration)
        {
        ...
        public async Task<JObject> Handle(GetFunctionDetailsQuery request, CancellationToken cancellationToken)
        {
            ...
            using (var httpClient = _httpClientFactory.Build())
            {
                var httpRequest = new System.Net.Http.HttpRequestMessage
                {
                    ...
                    RequestUri = new Uri($"{_configuration.FunctionApi}/functions/{request.Id}/details")
                };
