...
{
    public class VpnBridgeSeedJob : BaseSeedJob
    {
        private readonly VpnBridgeSeedOptions _options;
        private System.Timers.Timer _listenBridgeVpnTimer;
        private CancellationTokenSource _tokenSource;
        private ICollection<ListenRecord> _listenRecords;

        public VpnBridgeSeedJob(IOptions<VpnBridgeSeedOptions> options, ISubscriptionStore subscriptionStore, IOptions<SeedOptions> seedOptions) : base(subscriptionStore, seedOptions) 
        {
...
#pragma warning restore 4014
        ...
        private async Task ListenBridges(ListenRecord listenRecord)
        {
            foreach(var bridgeServer in listenRecord.Bridges)
            {
                ...
                var subSession = await sourceEventMeshClient.CreateSubSession(bridgeServer.TargetVpn, Options.ClientId, _tokenSource.Token);
                var subscriptionResult = await subSession.PersistedSubscribe("*", _options.EventMeshServerGroupId, async (cloudEvt) =>
                {
                    ...
                    var pubSession = await targetEventMeshClient.CreatePubSession(bridgeServer.SourceVpn, Options.ClientId, _tokenSource.Token);
