...
namespace FaasNet.EventMesh
{
    public partial class PartitionedEventMeshNode
    {
        ...
        {
            var vpn = await Query<VpnQueryResult>(VPN_PARTITION_KEY, new GetVpnQuery { Id = addClientRequest.Vpn }, cancellationToken);
            if (vpn == null) return PackageResponseBuilder.AddClient(addClientRequest.Seq, AddClientErrorStatus.UNKNOWN_VPN);
            var client = await Query<ClientQueryResult>(CLIENT_PARTITION_KEY, new GetClientQuery { Id = addClientRequest.Id }, cancellationToken);
            if (client != null) return PackageResponseBuilder.AddClient(addClientRequest.Seq, AddClientErrorStatus.EXISTING_CLIENT);
            var clientSecret = Guid.NewGuid().ToString();
            var addClientCommand = new AddClientCommand { Purposes = addClientRequest.Purposes, Vpn = addClientRequest.Vpn, ClientSecret = PasswordHelper.ComputePassword(clientSecret), SessionExpirationTimeMS = _eventMeshOptions.ClientSessionExpirationTimeMS };
