...
namespace FaasNet.EventMesh.Runtime.EF.Stores
{
    public class EFClientStore : IClientStore
    {
        ...
        public async Task<IEnumerable<Models.Client>> GetAllByVpn(string name, CancellationToken cancellationToken)
        {
            ...
            IEnumerable<Models.Client> result = await _dbContext.ClientLst.Where(c => c.Vpn == name).ToListAsync(cancellationToken);
        ...
        {
            await EventMeshDBContext.SemaphoreSlim.WaitAsync();
            Models.Client result = await _dbContext.ClientLst.FirstOrDefaultAsync(c => c.ClientId == clientId && c.Sessions.Any(s => s.Bridges.Any(b => b.Urn == bridgeUrn && b.SessionId == bridgeSessionId)), cancellationToken);
            EventMeshDBContext.SemaphoreSlim.Release();
            return result;
        }
        ...
        {
            await EventMeshDBContext.SemaphoreSlim.WaitAsync();
            Models.Client result = await _dbContext.ClientLst.FirstOrDefaultAsync(c => c.Vpn == vpn && c.ClientId == clientId, cancellationToken);
            EventMeshDBContext.SemaphoreSlim.Release();
            return result;
        }
        ...
        {
            await EventMeshDBContext.SemaphoreSlim.WaitAsync();
            Models.Client result = await _dbContext.ClientLst.FirstOrDefaultAsync(c => c.Id == id, cancellationToken);
            EventMeshDBContext.SemaphoreSlim.Release();
            return result;
        }
        ...
        {
            await EventMeshDBContext.SemaphoreSlim.WaitAsync();
            Models.Client result = await _dbContext.ClientLst.FirstOrDefaultAsync(c => c.ClientId == clientId && c.Sessions.Any(s => s.Id == clientSessionId), cancellationToken);
            EventMeshDBContext.SemaphoreSlim.Release();
            return result;
        }
        ...
        {
            var clients = await _dbContext.ClientLst.ToListAsync(cancellationToken);
